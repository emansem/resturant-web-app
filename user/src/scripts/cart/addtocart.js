...
const menuID = location.search.split("=")[1];
...
async function addNewProuctTotheCart(menuDetails, addTocartBtn){
    ...
    if(results.length !==0){
        ...
        const totaItems = localStorage.getItem('totaItems');
       let itemLength = Number(totaItems);
       orderCount.innerHTML = itemLength+=1
    } 
...
export async function getAllCartItems(){
    ...
    const results = await fetchDataFromDataBase('cart', 'customer_Id', customer_Id);
    if(results.length === 0){
        orderCount.innerHTML = 0;
        localStorage.setItem('totaItems', results.length);
    }else{
        console.log('there is and item in the cart', results.length);
        localStorage.setItem('totaItems', results.length);
       orderCount.innerHTML = results.length;
    }
